...
- 브라켓 공식 사이트(http://brackets.io) 에서 설치파일을 다운로드 한다.

![](./img/img18.png)

- [선택사항] Change 버튼을 선택하여 설치하고 싶은 폴더를 선택 > Next 버튼을 클릭한다.
...
- Brakets 실행 후 나타나는 Windows 보안 경고에서 '엑세스 허용' 버튼을 클릭한다.

![](./img/img22.png)



- 실행 후 '파일' 메뉴에서 '확장 기능 관리자'를 선택한다.

![](./img/img23.png)



- 검색창에 'NodeJS integration' 검색 > '설치' 버튼을 클릭한다.
...
- 적당한 위치에 폴더(앞으로 실습한 파일을 저장할 폴더)를 만들고  js파일로 저장하자(파일이름에 파일명.js 형태로 저장한다).

![](./img/img30.png)



- 화면 좌측의 'Getting Start'라고 표시된 부분을 클릭하여 '폴더 열기'를 선택한다.
...
- 실행화면 (앞으로 파일을 저장하면 좌측에 목록이 보일 것이다. 이후 실습에서는 js파일 저장 후 폴더를 선택하지 않아도 된다.)

![](./img/img33.png)



- 파일 목록에서 실행하고 싶은 파일을 우클릭 > 'Add to Node.js runner'를 선택한다.

![](./img/img34.png)



- 하단에 나타난 nodejs process sample 탭에서 드롭다운 메뉴(빨강)를 클릭한 후 파일명(파랑)을 선택한다.

![](./img/img36.png)



- 실행 버튼을 클릭 혹은 단축키(ctrl + shift + n)를 입력한다. (이후 코드를 수정하면 파일을 저장한 후에 실행 버튼을 클릭하면 실행된다.)

![](./img/img37.png)



- 코드가 실행되었음을 확인 할 수 있다.
